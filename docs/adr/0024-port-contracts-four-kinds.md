...
manifest 上的适用前提、以及业务侧代码的注释。三处都是给人读的字串，没人在第一帧查它。
结果是下游拟合算子在 2327 项失败之后，靠 grep 才找到「原来这里要求 1280 槽」这句话。

适用前提（M5 加在 manifest 上的自由文本字段，M7 已删除）不会在运行时拦住任何东西。
空槽 bug 需要的是反过来的东西：一个**能在数据到达的那一刻就检查**的不变量。
...
- **把所有约束都塞进契约**。写不成这四种之一的，只依赖参数与连接关系的进算子的 `validate`
  钩子（M7，加载期），依赖数据语义的做成运行期信号（quality 字段 / warn 日志 / 错误值），
  见 `docs/op-packs.md`「约束写在哪里」。
