...
所以单独验它给出的是「该节点无点云输出」而不是一片空白。（后来改成借上游最近的那片云当底图，
这条规则现在由 `scripts/e2e/gap.mjs` 量测输出分组里的「底图」几条断言验。）
...
**怎么跑**：`pnpm e2e`（分组「改参数 → 结果标为过时」）。2026-09-26 起这一组删掉了：M3 起 stale 只标受影响的
节点，由 `m3.mjs`「1.1 缓存复用」分组的「改完之后 stale 精确到那两个节点」「画布上恰好两个节点是虚线框」
「工具栏显示「已过时」」覆盖。
...
§11 没有单列，但数据通路是 M2 做的，顺手验了（2026-09-26 起这一组删掉了：右键「运行到此节点」由
`noderun.mjs` 验，只跑上游闭包由 `m3.mjs`「P1 #27 + E7：Shift+F5 跑到选中节点」验）：
