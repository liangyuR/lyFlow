...
- chip 计数（全部 / 已改动 / 配方 / 诊断）逐个相等；每个 chip 下把虚拟列表从头滚到尾收齐的行键 = 期望集合；
- 搜索「gain」「体素」「demo-tag」「参数全类型 iter」（多词都要中）「LEAF」（大小写不敏感）的结果与计数；
- 类型 vec3f、vec3f + 已改动，「类型 ▾」下拉里的计数叠着当前 chip；
...
改成等视图显示出那片云再读；排查时顺带发现下面「P2 之外发现的问题」那一条，把 ROI 分组挪到库算子分组前面。改完之后
...
- **重扫库目录之后第一次运行取不到结果**：`refresh_library`（Tauri 命令）先 `runs.drop_all()`；之后的第一次运行里节点全部报
  `skipped`（命中缓存），但 `getOutputCloud(runId, …)` 返回「core 没有该结果」，3D 视图空白；再跑一次就好了。复现：存库并重扫
  （P1 / P2 的库算子分组都会做）→ 跑一张以前跑过的图 → 选中节点看 3D 视图。与参数面板无关（core / bridge 的结果挂载），
  e2e 里把要 2D 底图的 ROI 分组排在库算子分组前面避开，没有改 bridge。
