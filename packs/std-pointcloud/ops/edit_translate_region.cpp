...
void registerEditTranslateRegion(Registry& r) {
  ...
  op.preconditions = {
      ...
      "壁上一半的点会跟着动；切分面压在下游要用的锚点上时，锚点会跟着动而读出「完美跟随」"
      "—— 两种都会得出错误的灵敏度结论，选区必须先在剖面上核对。",
      "半空间的法向不要求归一化，只用它的方向；零向量是 bad_param。",
      "平移不改点数也不改点序，下游按下标对账的算子（Indices）仍然成立。",
      "法向通道不跟着转：本算子只做平移，不做旋转。",
  };
