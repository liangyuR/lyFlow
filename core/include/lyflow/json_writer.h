#pragma once
//
// 最小 JSON 写出器。刻意不引第三方库：core 在 M0 阶段应当零依赖，
// 这样「加算子 -> 前端出现」这条链路的构建时间是秒级的，不会被
// vcpkg 拉 boost/VTK 卡住。等 M2 真的要跑点云了再谈依赖。
//
// 两条硬要求（见 docs/graph-doc.md「可稳定序列化」）：
//   1. 键顺序由调用方决定且固定 —— 所以这里是流式写出，不是 map。
//   2. 浮点数不产生噪声 —— 用最短往返表示，2.0 写成 2.0 而不是 2.0000000001。
//
#include <cstdint>
#include <string>
#include <vector>
