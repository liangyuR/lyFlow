...
TEST_CASE("select / selectInverse：搬全通道、不造通道、越界跳过、互为补集") {
  SUBCASE("保留全部通道") {
  ...
  SUBCASE("selectInverse 是 select 的补集") {
...
TEST_CASE("bounds：跳过非有限点，空点云不 valid") {
  SUBCASE("跳过非有限点") {
  ...
  SUBCASE("空点云的 bounds 不 valid") {
    const PointCloud c;
    CHECK_FALSE(c.bounds().valid);
  }
}

TEST_CASE("Data 的类型标签与 manifest 端口类型一一对应，2D 载荷类型名往返") {
  SUBCASE("核心类型") {
...
TEST_CASE("valueJson：Box2D / Point2D / Circle2D / Record 各写各的字段，点云与 Indices 不走 valueJson") {
  SUBCASE("Box2D / Point2D") {
    Box2D b;
    b.min[0] = -0.5f;
    b.min[1] = 0.25f;
    b.max[0] = 1.5f;
  ...
  SUBCASE("Record 原样带着算子包定义的 JSON") {
    Record r;
    r.type = "GapAlignment";
  ...
  SUBCASE("点云与 Indices 不走 valueJson") {
...
TEST_CASE("Tensor 只把形状与统计量写进 valueJson，全是非有限值时统计量写成 null") {
  SUBCASE("只把形状与统计量写进 valueJson") {
    Tensor t;
    t.shape = {2, 3};
  ...
  SUBCASE("全是非有限值时统计量写成 null 而不是坏 JSON") {
    Tensor t;
    t.shape = {2};
